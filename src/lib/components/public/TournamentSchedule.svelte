	...
	{#if days.length > 1}
		...
		<div class="mb-6 flex flex-wrap items-center justify-center gap-2">
			<button
				...
				class={cn(
					'cursor-pointer rounded-pill border px-4 py-1.5 text-[11px] font-bold uppercase tracking-widest transition-colors duration-100',
					selected === 'all'
					...
					<ul class="divide-y divide-border">
						{#each daySlots as s (s.id)}
							<li class="flex flex-wrap items-center gap-x-4 gap-y-2 px-5 py-3.5">
								<span class="w-14 shrink-0 font-display text-[15px] tabular-nums text-primary">
									{timeLabel(s.starts_at)}
								</span>
								<span class="w-24 shrink-0 text-[11px] font-bold uppercase tracking-[0.08em] text-muted">
									{s.court_name}
								</span>
								...
								<div class="min-w-0 flex-1 basis-40">
									{#if s.event_name}
										<span
											class="mb-1 inline-block rounded-pill border border-border bg-subtle px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-muted"
										>
											{s.event_name}
										</span>
									{/if}
								...
								{:else if s.match_status === 'completed'}
									<span class="shrink-0 text-[11px] font-bold uppercase tracking-[0.08em] text-muted">
										Final
									</span>
								{/if}
							</li>
